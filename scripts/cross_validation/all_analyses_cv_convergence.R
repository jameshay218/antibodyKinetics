...
res_wd <- top_wd <- "~/Drive/Influenza/Ferret/PLOS Comp Bio/cross_validation_reruns/"
...
#convergence_check <- read.csv("~/Drive/Influenza/Ferret/PLOS Comp Bio/cross_validation/convergence_check.csv",stringsAsFactors=FALSE)
#rerun_ids <- convergence_check[convergence_check$rerun == TRUE,"runID"]
#runs <- runs[runs$runID %in% rerun_ids,]
###
...
for(i in 1:nrow(runs)){
    ...
    gelman_multi <- tryCatch({
        ...
        pdf(paste0(res_wd,"/",runID,"_",runName,"_gelman_plot.pdf"))
        gelman.plot(chains_multi[["list"]])
        dev.off()
    }, warning = function(w){
        print(w)
    }, error = function(e){
    ...
    if(do_model_comparison){
        ...
        dat <- as.matrix(tmp_dat[,4:ncol(tmp_dat)])
        dat <- rbind(times, dat)
        ...
        for(i in 1:nrow(chain1)){
          pars <- get_index_pars(chain1,i)
          y <- f(pars, times)
          tmp_liks[i] <- log(norm_error(y[cv_row_model,cv_col], cv_data_point,pars["S"],pars["MAX_TITRE"]))
        }
